...
# ---------------------------------------------------------------------------
# These MUST match AmelTechBot.h exactly (AMELTECH_MAX_QUESTION_LEN,
# AMELTECH_MAX_ANSWER_LEN, AMELTECH_MAX_CATEGORY_LEN). A mismatch here means
# entries that pass validation could still overflow fixed-size buffers sized
# from the C++ macros at runtime on-device.
MAX_QUESTION_LEN = 96        # must match AMELTECH_MAX_QUESTION_LEN in AmelTechBot.h
MAX_ANSWER_LEN = 220          # must match AMELTECH_MAX_ANSWER_LEN in AmelTechBot.h
# NOTE: AMELTECH_MAX_CATEGORY_LEN in AmelTechBot.h is currently 24, but the
# longest name in VALID_CATEGORIES below ("football_player_celebrity_profile")
# is 33 chars. 24 is structurally too small for the whitelist and must be
# raised in AmelTechBot.h to MAX_CATEGORY_LEN below (34, to include the null
# terminator headroom conventionally used elsewhere in this file).
MAX_CATEGORY_LEN = 34         # must match AMELTECH_MAX_CATEGORY_LEN in AmelTechBot.h
MAX_ENTRIES = 7000
...
def validate_dataset(data):
    ...
    for idx, entry in enumerate(data):
        ...
        # Required fields — only look at THIS entry's missing fields, not
        # errors accumulated from earlier entries.
        missing = [f for f in ("question", "answer", "category") if f not in entry]
        if missing:
            for field in missing:
                errors.append(f"{loc}: missing required field '{field}'")
            continue  # can't safely run further checks without all three
        ...
        if len(category.strip()) == 0:
            errors.append(f"{loc}: category is empty")

        # Length limits (must match the C++ firmware macros — see header above)
        ...
        if len(category) > MAX_CATEGORY_LEN:
            errors.append(
                f"{loc}: category exceeds MAX_CATEGORY_LEN={MAX_CATEGORY_LEN} "
                f"({len(category)} chars): '{category}'"
            )
